...
Deno.serve(async (req) => {
  try {
    ...
    if (action === 'list') {
      const items = await base44.entities.TelephonyQueue.filter({ org_id });
      return Response.json({ items });
    }

    if (action === 'create') {
      const item = await base44.entities.TelephonyQueue.create({ ...payload, org_id });
      return Response.json({ item });
    }

    if (action === 'update') {
      if (!id) return Response.json({ error: 'id required' }, { status: 400 });
      const item = await base44.entities.TelephonyQueue.update(id, payload);
      return Response.json({ item });
    }

    if (action === 'delete') {
      if (!id) return Response.json({ error: 'id required' }, { status: 400 });
      await base44.entities.TelephonyQueue.delete(id);
