...
def test_appointment_request_create_allows_staff_placeholder(monkeypatch):
    ...
    monkeypatch.setattr("main._require_test_page_role", lambda endpoint_path, firebase_user, roles, **kwargs: rbac_context(["staff"]))
...
def test_appointment_request_create_allows_admin_placeholder(monkeypatch):
    patch_appointment_auth(monkeypatch)
    monkeypatch.setattr("app.services.appointment_request_service.SessionLocal", None)
    monkeypatch.setattr("main._require_test_page_role", lambda endpoint_path, firebase_user, roles, **kwargs: rbac_context(["admin"]))
...
def test_appointment_request_create_allows_provider_placeholder(monkeypatch):
    patch_appointment_auth(monkeypatch)
    monkeypatch.setattr("app.services.appointment_request_service.SessionLocal", None)
    monkeypatch.setattr("main._require_test_page_role", lambda endpoint_path, firebase_user, roles, **kwargs: rbac_context(["provider"]))
...
def test_appointment_request_list_allows_viewer_read_only(monkeypatch):
    patch_appointment_auth(monkeypatch)
    monkeypatch.setattr("app.services.appointment_request_service.SessionLocal", None)
    monkeypatch.setattr("main._require_test_page_role", lambda endpoint_path, firebase_user, roles, **kwargs: rbac_context(["viewer"]))
